    ...
    switch (id) {
      ...
      case "classFeedback":
        ...
        if (!strUser) {
          ...
          if (strUser1 == "Application Engineering Development") {
            alert("APPLICATION ENGINEERING DEVELOPMENT SELECTED");

            txtdisplay.style.visibility = "visible";
            var cl1 = document.getElementById("classes");
            cl1.innerHTML =
              '<input type="checkbox" name="checkprof" id="checkprof1" value="Prof 1" onclick="onPresscheckbox(this)" style="display: inline;"/>Professor 1';
          }


        
          if (strUser1 == "User Interface and Web Development") {
            alert("USER INTERFACE AND WEB DEVELOPMENT SELECTED");
            txtdisplay.style.visibility = "visible";
            var cl1 = document.getElementById("classes");
            cl1.innerHTML =
              '<input type="checkbox" name="checkprof" id="checkprof2" value="Prof 2" onclick="onPresscheckbox(this)" style="display: inline;"/>Professor 2';
          }
          if (strUser1 == "Data Science") {
            alert("DATA SCIENCE SELECTED");
           
            txtdisplay.style.visibility = "visible";
            var cl1 = document.getElementById("classes");
            cl1.innerHTML =
              '<input type="checkbox" name="checkprof" id="checkprof3" value="Prof 3" onclick="onPresscheckbox(this)" style="display: inline;"/>Professor 3';
          }
          if (strUser1 == "Cloud Technologies") {
            alert("CLOUD TECHNOLOGIES SELECTED");
            txtdisplay.style.visibility = "visible";
            var cl1 = document.getElementById("classes");
            cl1.innerHTML =
              '<input type="checkbox" name="checkprof" id="checkprof4" value="Prof 4" onclick="onPresscheckbox(this)" style="display: inline;"/>Professor 4';
          }
          if (strUser1 == "Encp") {
            alert("ENCP SELECTED");
            txtdisplay.style.visibility = "visible";
            var cl1 = document.getElementById("classes");
            cl1.innerHTML =
              '<input type="checkbox" name="checkprof" id="checkprof5" value="Prof 5" onclick="onPresscheckbox(this)" style="display: inline;"/>Professor 5';
          }
        }
...
function onClassChange() {
            const classSelect = document.getElementById('classFeedback');
            const selectedValue = classSelect.value;
            const textField = document.getElementById('thetextfield');
            const classesDiv = document.getElementById('classes');

            // Show classes if a valid class is selected
            if (selectedValue !== "selectone") {
                classesDiv.style.visibility = 'visible';
            } else {
                classesDiv.style.visibility = 'hidden';
                textField.style.visibility = 'hidden'; // Hide text field when no class is selected
            }
        }

        function onCheckboxChange() {
            const checkboxes = document.querySelectorAll('input[name="checkprof"]');
            const textField = document.getElementById('thetextfield');

            // Check if any checkbox is checked
            const anyChecked = Array.from(checkboxes).some(checkbox => checkbox.checked);

            // If no checkbox is checked, hide the text field
            if (!anyChecked) {
                textField.style.visibility = 'hidden';
            } else {
                textField.style.visibility = 'visible'; // Show text field if at least one checkbox is checked
            }
        }
